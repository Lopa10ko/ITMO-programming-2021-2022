...
int main() {
    ...
    char *error_msg = 0;
    if (int connect = sqlite3_open("currency.db", &db)) {throw std::invalid_argument("ERROR: Database not connected");}
    ...
    if(curl) {
        ...
        sql_script = "INSERT INTO main.currency_list (id, curr_name, median, mean_span, value_1) " \
                     "VALUES (4, 'eWR', 32, 23, 110);";
        std::cout << sql_script << std::endl;
        sqlite3_exec(db, sql_script.c_str(), callback, 0, reinterpret_cast<char **>(error_msg));
        sql_script = "SELECT * FROM 'main.currency_list';";
        sqlite3_exec(db, sql_script.c_str(), callback, 0, &error_msg);
